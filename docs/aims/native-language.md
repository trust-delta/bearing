...
⚠ **2026-09-03、人間が観測し `state: done` を宣言した —— だが閉じたのは到達であって前提ではない。** 観測されたのは*書かれたものが読めること*であり（人間は散文の register を見て「引き続きこの言語テイストで推移してもらいたい」と述べた）、⚠ **「混在より統一のほうが質が高い」が実測されたわけではない。** 上の核心はいまだ得られていない ∴ **`done` は反証を閉じない** —— 混在が無害だと分かれば、そのとき動くのはやはり手段のほうである。
