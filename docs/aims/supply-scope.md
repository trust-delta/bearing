...
⚠ **置いた後に問われるのは「常に最新を強制するか」である**（人間が 2026-09-04 に指摘）。⚠ **最初の実装は「人間が直した」と「版が古い」を『中身が違う』の一語に畳んでいた** —— **`CLAUDE.md` の block は marker が版と本文 sha を運ぶので既にこの 2 つを分けている** ∴ **同じ repo の中で、片方だけが解かれていた。** ⚠ **canon の file に marker は挿せない**（挿せば中身が書き換わり、bearing 自身の正本と食い違う）∴ **足場を file の外に置く** —— `_guide/.bearing-canon.json` が、我々が最後に置いた枚とその sha を記録する。

⚠ **∴ 判定は 5 値になり、更新は「我々が置いたまま」のときだけ黙って走る**（人間の決定 2026-09-04）—— `stale` は上書きが安全である証拠を台帳が持っており、`edited` と `unknown` は持たない。⚠ **canon はエージェントが従う法である** ∴ **古いまま黙っていること自体が drift であり、追随しないことは中立ではない。**
...
- [done] **`/bearing:with-aim` が canon を置き、我々が置いたままなら最新へ追随させるようにした。** 置くのは **2 枚**（`aim-authoring.md` / `aim-facts.md`）—— ⚠ **aim の canon だけである。** ⚠ **`handoff.md` は入れない**（人間が 2026-09-04 に正した）—— **ここは aim の opt-in が置く場所であり、入れれば handoff の canon が aim の採用に依存する。** [[session-handoff]] は `with-aim` 無しで動かねばならない ∴ その分離を実装が破ることになる。⚠ **入れる必要も無い**: handoff の skill は自分の同梱物を裸の名で指しており、repo 側の `_guide/handoff.md` を 1 度も要求しない（実測 2026-09-04）。`frame.md` は hook と block が運び、`_guide/README.md` は著述する側の doc である。⚠ **既に在って中身が違う枚は触らず、触っていないことと理由を同じ息で述べる** —— 置いた後の `_guide/` はその repo の doc である。⚠ **比較の前に改行を正規化する** —— `core.autocrlf=true` の機体では checkout が CRLF へ変え、素朴な比較は**中身が同じ file を「違う」と呼ぶ**。そして「違う」は人間を呼び出す合図ゆえ、**偽陽性はそのまま雑音になる**。⚠ **同梱物が読めないときはそう述べる**（「置かなかった」と「置く元が無かった」を同じ沈黙にしない）。⚠ **判定は 5 値である**（`place` / `current` / `stale` / `edited` / `unknown`）—— 足場は `_guide/.bearing-canon.json`（我々が最後に置いた枚と sha）。⚠ **書くのは `place` と `stale` だけ。** ⚠ **中身が正本と同じなら台帳が無くても `current`** —— 手で正しく置いた repo を「由来不明」と呼べば、正しい状態が警告を出し続ける。⚠ **台帳が壊れていたら記録が無いものとして扱う**（読めない記録を根拠に他人の file を上書きしない）。⚠ **触らなかった枚に今の正本の sha を書かない** —— 次の実行がそれを「我々のまま」と読んで人間の編集を踏む。断る道は `--no-canon`、⚠ **`--remove` は canon を消さない** —— opt-in を外すことと、その repo が持つ doc を捨てることは別の act である
