...
⚠ **候補が 1 つ現れた**（⛔ 実装しない・⛔ 推薦しない）—— 🔴 **`last-visit` が選んだ `根拠:` は、canon の慣例ラベル（`依存:` / `関連:`）に無い。** **canon は列挙が閉じているか開いているかを述べていない** ∴ **消費者が作った label が、後日こちらの予約語になれば同じ衝突が再発する** —— **`検証:` で起きたのがまさにそれである。** ⚠ **閉じると述べれば消費者の自由を奪い、開くと述べれば衝突の余地を認めることになる** ∴ **選ぶのは人間である。**
