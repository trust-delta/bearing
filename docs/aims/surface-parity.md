...
- [done] **同じ事実を運ぶ 2 つの面が、同じ述語で黙るようにした。** hook は `CLAUDE.md` の marker を見て黙るのに、statusline は corpus の有無しか見ておらず、**aim を採っていない全 project に 2 行目を描いていた**。⚠ **これは「どの面に何が出るか」の非対称ではなく、*同じ面同士の食い違い*である** —— この node が測ってきた欠落とは別の種であり、**user スコープで載せるまで見えなかった**（採った repo でしか面を見ていなかった）。手段は [[ambient-display]] が持つ
