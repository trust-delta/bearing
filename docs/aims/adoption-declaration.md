...
- [done] **`setup-aim` が block と skill を 1 組として扱うようにした。** 🔴 **判定をすべて済ませてから書く** —— 先に block を書いて skill で止まれば、**止まったのに片方が動いた状態が残る**。⚠ **skill の一致は中身だけで見る**（状態も台帳も持たない。台帳は `# HISTORY` で棄却済み）が、**改行だけは正規化する** —— CRLF の checkout では git が変換しただけで「一致しない」になり、**同じ轍を block の sha で既に踏んでいる**（[[bearing]]）。⚠ **欠けている 1 枚も「一致しない」に数える** —— **半分だけ置かれた skill は、古い skill より静かに壊れている。** ⚠ **`--check` の終了値は「組」の判定へ変えた** —— 旧い code は *block だけ* を見て「別の軸を混ぜれば呼ぶ側が何が赤いのか分からない」と述べていたが、🔴 **その理由は軸が 2 つ在ることに乗っていた。1 組になった今、軸は 1 つである。** ⚠ **採っていない repo は赤くしない**（`absent`）—— 未採用は異常ではない。✅ **変異試験で確かめた**（実測 2026-09-07、gate を `false &&` で潰し `overwrite` を落とすと 15 本中 3 本が赤、復元で 15 本緑）。⚠ **`block を人間が編集していたら skill も置かれない` の 1 本は、直す前も緑だった** —— 旧い code も refuse を skill の前で return していた ∴ **あれが固定するのは新しい挙動ではなく、組の判定を足しても壊れないことである。** ⚠ **再測は `node --test test/setup-aim.test.mjs`（15 本）と `node scripts/consumer-check.mjs`（27 件。⑽ が「block も置かれない」、⑾ が「`--update` は両方を揃える」）である**
