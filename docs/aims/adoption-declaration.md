...
⚠ **2026-09-05 まで、実装はこの本文より寛容だった。** 述語は `adopted || hasCorpus` であり、**採用を宣言していない repo でも corpus さえ在れば機構が口を開いていた** —— 理由は移行の便宜（既に node を書いている project を、印が無いという理由で黙らせない）である。⚠ **同日、人間が推測をやめた**（下段）—— **本文が最初から述べていたことに、実装が追いついた形である。**
...
⚠ **project scope の `false` は、user scope で載った plugin を覆す**（実測 2026-09-05、Claude Code、`claude plugin list`）—— `.claude/settings.json` に `{"enabledPlugins": {"bearing@trust-delta": false}}` を置いた repo は `✘ disabled`、置いていない repo は `✔ enabled` と出た。⚠ **測ったのは `plugin list` の報告であって、session で hook が黙ることそのものではない** —— 一段手前の観測である。再測は上の 2 つを並べて打つだけ。

∴ **「この repo では bearing を載せない」は実在する手である。** ⚠ **だが降りる宣言と競合しない —— 落とす範囲が違う。** あの switch は**全部**を落とす（handoff も、[[ambient-display]] の 1 行目も、drift の検知も）が、`--decline` は **aim の 4 面だけ**を黙らせる。⚠ **束が一体であるほど、片翼だけ降りる手が要る** —— 丸ごと落とす手しか無ければ、**aim を通したくない repo は baton まで手放すことになる**（handoff は `docs/aims/` に何も依存せず、痕跡も残さない ∴ その repo が一度も反対していないものである）。⚠ **加えて、あの switch が repo の宣言になるのは `.claude/settings.json` が tracked のときだけである** —— `.claude/` を丸ごと ignore する repo では machine-local な事実にとどまり、**bearing 自身が 2026-09-05 まではその側だった。**
...
⚠ **「降りる宣言」を置き、同じ日に撤去した。** 字面は `CLAUDE.md` の単独行 `<!-- bearing:aim declined -->` で、`--decline` が置いた。⚠ **必要だったのは、述語が corpus を見ていたからである** —— corpus を持つ repo は block を外しても黙らず、降りるには専用の「否」が要った。∴ **推測をやめた瞬間に、覆すための機構も要らなくなった**（人間の決定 2026-09-05）。

⚠ **消えたのは 4 つである**: `--decline` そのもの／「宣言していない」と「降りた」の 2 状態／`--remove` が「黙るようになる」と言えなかったこと／[[session-handoff]] とは無関係に増えていた 3 つ目の面の分岐。**どれも同じ 1 つの OR が生んでいた。**

⚠ **旧い宣言が残った repo は、何もしなくても正しく黙る** —— あの行は今や既定と同じ意味（＝ 採用していない）を持つからである。**移行のための特別扱いは 1 行も要らない。** ⚠ **ただし 1 つだけ手当てが要る**: あの行は `bearing:aim` で始まる ∴ **除外しなければ「読めない marker」＝ anomaly と判定され、anomaly は採用の事実として数えられる** —— **降りたはずの repo が有効になり、意味がちょうど反転する。**

⚠ **撤去した flag は、黙って無視してもいけない。** `--decline` を素通りさせれば adopt の経路へ落ち、**降りるつもりで打った repo が採用される。** 述べて、書き換えず、非 0 で終わる。

HTML コメントゆえ、この宣言は消費者の context を 1 token も食わない。
...
⚠ **`precompact` の 2 条件は対等ではない。** baton が在ることは **handoff 自身の証拠**であり、aim の採用は aim の証拠でしかない ∴ **aim を採っていなくても、baton が在れば発火する。** 上段の唯一の例外が、この面における形である —— ここで黙らせることは aim の沈黙ではなく handoff の欠落になる。⚠ **2026-09-05、もう 1 つの条件は corpus から採用の宣言へ動いた** —— **非対等であることは動いていない。**

⚠ **`--remove` は嘘をついていた —— そして嘘でなくなった**（どちらも 2026-09-05）。block を外したあと「hook はこの project で黙るようになる」と*断言*していたが、述語が `corpus 在り || marker 在り` である以上、corpus を持つ repo は外しても黙らなかった。⚠ **まず述べ分けるよう直し、次に述語から corpus が落ちて、断言が真になった。** ⚠ **同じ 1 文が、実装の変更 2 回を挟んで嘘 → 正直 → 断言と動いている** —— 直したのは文ではなく、文が指していた事実のほうである。
...
⚠ **置かれた project skill は、同じプロセスの中で live に登録される**（実測 2026-09-05、Claude Code、bearing の checkout で `setup-aim` を打った直後、再起動なしに harness の skill 一覧へ `aim` が現れた）。⚠ **これを「plugin の skill 一覧はプロセスで固まる」との対照として書いていたが、その裏付けは弱かった** —— [[bearing]] の実測 2026-09-04 が測ったのは *slash command* の面であり、**skill については推定である**（あちらが自ら限界として名指している）。🔴 **2026-09-06、その推定は否定された**: **既に一覧に在る plugin skill の実体は、同一プロセスの中で新しい版へ追随した**（[[bearing]] の実測 2026-09-06）。⚠ **未測として残るのは「起動後に置いた plugin の skill が一覧に現れるか」だけである** ∴ **対照は立っても倒れてもいない** —— **2 つの一覧が別々の時点を指しうるかは、そこが測られるまで述べられない。** ∴ 本 node が実際に持っているのは project skill 側の観測だけであり、**単一観測であり、どの時点で拾われるか（file の出現か、次の turn か）は分けられていない。** 再測は安い: 別 repo で `setup-aim` を打ち、skill 一覧を見る。
...
⚠ **bearing は、置かれたものを track する側を選んだ**（人間の決定 2026-09-05）。`.gitignore` から `.claude/*` を外し、machine-local な `.claude/**/*.local.json` だけを名指しで閉じた。⚠ **これは上段の「置いた後は repo のもの」と矛盾しない —— その行使である**: track するかは repo の policy であり、bearing は 1 つの repo としてそれに答えた。⚠ **極性が反転した以上、危険も反転する** —— 2026-09-02 に丸ごと ignore したのは「調査ノートが黙って公開されかけた」からであり、理由は「知らないものは安全側へ倒す」だった。**今は閉じ忘れが公開になる** ∴ その警告を `.gitignore` の見出しへ移した。⚠ **そして腐りうる対が 1 つ増える** —— 置かれた 3 枚は `templates/aim/` からの複製である ∴ `test/placed-skill-sync.test.mjs` が byte 同一を見る。**これは plugin の義務ではなく**（plugin は追随させない）、**この repo の policy である。**

⚠ **反転は前提から出た**（人間が 2026-09-05 に述べた）。bearing は aim / handoff / drift の検知をもって**バイブコーディングの質と速度を上げ、人間の注意予算を真に価値のある判断点にだけ使わせる**ためにある ∴ **基本的にどの repo でも aim を使いたい** —— だが user scope に載せると、**aim を使っていない repo に向くこと自体へ大きな抵抗が掛かる**。共同開発の場ではなおさらである。⚠ **回避は在る**（project scope の `enabledPlugins: false`。実測 2026-09-05、上段）—— **だがそれは束を丸ごと落とし、handoff も drift の検知も一緒に消える。** ∴ **回避の手段が「採用した project でだけ aim 機能を有効にする」ことである。**

⚠ **そして「採用した」を corpus の存在から推測してはならない。** 推測すれば、**team が採っていない共同 repo で機構が黙って喋る** —— 避けようとした抵抗が、1 段内側で再現する。∴ **述語は `adopted` だけになった**（`lib/claude-md.mjs` の `isEngaged`）。

⚠ **代償は 1 つだけで、それは発見面である。** 採用するまで機構は完全に黙る ∴ **黙る機構は自分の存在を告げられない。** [[ambient-display]] の 2 行目が、corpus を見つけたことだけを 1 行述べて埋める。⚠ **警告色は与えない** —— 採っていないことは構造的に正常であり、色で重さを付ければ**採用を促す面**になる。それは注意予算の割り当てであって観測ではない。
...
- [done] **述語から corpus を落とした —— aim の機構が口を開くのは、採用を宣言した project だけである。** `lib/claude-md.mjs` の `isEngaged` は `adopted` だけを見て、**4 つの面すべてがそこを通る**（`aim-facts` / `statusline` / `corpus-delta` / `precompact`）。⚠ **`precompact` だけは baton が在れば採っていなくても発火する** —— baton は handoff 自身の証拠であり、2 条件は対等ではない。⚠ **同じ 1 つの OR が生んでいた 4 つが同時に消えた**: `--decline`・3 つ目の状態・`--remove` の但し書き・`--check` の分岐。⚠ **旧い「降りる宣言」は既定と同じ意味になる ∴ 特別扱い無しで正しく黙る** —— ただし **anomaly に落とせば意味が反転する**（anomaly は採用として数えられる）ので除外は残す。⚠ **撤去した `--decline` は素通りさせない** —— 落ちれば adopt の経路へ行き、**降りるつもりで打った repo が採用される**。`test/aim-adoption-gate.test.mjs` の 13 件と、合成消費者の job の 4 件が固定する
- [done] **黙る機構が自分の存在を告げる 1 行を、[[ambient-display]] の 2 行目に置いた。** corpus を見つけたが採用されていないとき、`aim 未採用 / corpus N` と述べる。⚠ **corpus が無ければ完全に黙る** —— `docs/aims/` を持たない repo に「未採用」と述べれば、それは全 project に居座る勧誘の行になる。⚠ **警告色は与えない**（採っていないことは構造的に正常である）
...
- 関連: [[consumer-evidence]] —— 本 node が**何を置かないか**を決め、あちらが**置かなかった結果が向こうでどう見えるか**を検める。⚠ **canon を置くのをやめた以上、消費者に届くのは `CLAUDE.md` の block と `.claude/skills/aim/` の 2 つだけである**（本 node の `# IS`。以前ここは「block だけ」と書いていた —— 同じ node の中で食い違っていた）∴ あちらの検査は安全網として本 node に依存しており、**2 つとも検めている**
