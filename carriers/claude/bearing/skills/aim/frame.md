...
⚠ **この repo が aim corpus を持つなら、開発はそれによって駆動される。** `{{aims}}/<slug>.md` の各ファイルが 1 つの aim（目的とその手段）であり、親子で目的を分解した木を成す。


- **aim に触れる前に [`{{aims}}/_guide/aim-authoring.md`](aim-authoring.md) を読むこと。** slug の付け方、body の section、木の保守、drift の検出と修復は、そこが唯一の正本である。
