...
2. **`composed-at` と `read-at` を控える。**
   ...
   - 🔴 **`transcript:` が在れば、それが前の対話の原文の在り処である。** ⚠ **開くのは、逐語・error の原文・生成した code が要るときだけ** —— **baton は「何が起きたか」を運ぶが、原文は運ばない** ∴ **ここが唯一の出所である。** ⚠ **無いこともある**（古い baton・path が解決しなかった場合）—— **無いことを欠陥と読まない。**
