...
⚠ **同じ 2 つを、引数なしで選べる別名が在る** —— `/bearing:handoff-r` と `/bearing:handoff-w`。🔴 **理由は入口であって儀式ではない**: 引数つきの形は**サジェストから選んでもそのままでは打てず**、`r` / `w` を人間が手で足すことになる（人間の指摘 2026-09-07）。⚠ **あの 2 枚は手順を 1 文字も持たず、この skill を引数つきで呼ぶだけである** —— **正本を 2 つにしないための形である。** ⚠ **かつてこれは `r` / `w` の 2 skill だった** —— **2 枚が同じ 110 行を運んでいた** ∴ 1 枚へ畳んだ（2026-09-05）。🔴 **拒まれたのは「canon を 2 つ持つこと」であって「入口が 2 つ在ること」ではない** —— 中身を持たない別名は、その理由に触れない。
