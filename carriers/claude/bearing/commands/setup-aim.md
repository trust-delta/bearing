...
🔴 **plugin を update した直後なら、順序が要る —— `/plugin` の update → `/reload-plugins` → このコマンド。** ⚠ **理由は、走る複製が自分の隣の `templates/` を置くことである** —— **PATH が古い版を指したままこのコマンドを打てば、*古い法が置かれ、しかも成功として報告される*。** 🔴 **新しい版が同じ cache に在ることを、CLI は述べない。**

⚠ **壊れた顔をしないことが、この順序を書く理由である。** **不可逆ではない**（順に打ち直せば直る）**が、沈黙している** ∴ **人間は緑の報告を受け取り、更新が届かなかったことを知る手を持たない**（実測 2026-09-17、対象: この機体 —— `0.30.0` の CLI で置くと `v0.30.0` の法が置かれ、cache に在る `0.32.0` には 1 語も触れなかった。⚠ **測ったのは 1 台であり、cache の並びが版ごとの dir であることは docs で確かめていない**）。

⚠ **`/reload-plugins` を打つのは人間である** ∴ **エージェントはここで止まり、順序を述べる。** 🔴 **CLI の出力に出た版が、人間が期待した版と違うなら、それを黙って通してはならない** —— **述べるところまでがこちらの仕事である。**
