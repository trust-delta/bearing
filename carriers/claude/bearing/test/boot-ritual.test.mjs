...
// 義務をちょうど一度だけ述べること、そして operator が入力したものに決して触れないこと。
...
test('it never exits 2 — the operator’s prompt is never erased', async () => {
  const root = await unitWithBaton()
  try {
    // UserPromptSubmit での exit 2 は「処理を遮断し、元の prompt を消去する」。⚠ operator に
    // 仕えるための儀式を強制するために operator が入力したものを破壊することは、
