...
test('a corpus yields the fences and the open-todo count', async () => {
  ...
  const out = compose(root)
  assert.match(out, /```tmai-aim-drift-intra v1/)
  assert.match(out, /```tmai-aim-drift-inter v1/)
  assert.match(out, /```tmai-aim-working-delta v1/)
  assert.match(out, /```tmai-aim-unpushed v1/)
  assert.match(out, /```tmai-aim-checkpoint-stale v1/)
