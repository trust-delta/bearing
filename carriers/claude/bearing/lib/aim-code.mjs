 ...
 * **慣例ラベル（`依存:` / `関連:`）と混ぜてはならない**（正本は `aim-authoring.md`）。
 *
 * ⚠ **当初この label は「`aim-authoring.md` が既にその名を慣例ラベルに挙げている」ことを理由に
 * 選ばれた** —— そのため 1 つの語が「関係の宣言」と「drift の片付け」を兼ね、`td-apps-site` の
 * corpus で 2 つの意味が同じ節に同居した（報告 2026-09-15）。⚠ **機械は下の `@` の有無で分けており
 * 取り違えていない** —— 🔴 **危うかったのは読み手のほうで、句読点で意味を見分けることになっていた**
 * ∴ 慣例ラベルの側から `検証:` を外した（人間の決定 2026-09-15）。**門は `aim-code.test.mjs`。**
 *
