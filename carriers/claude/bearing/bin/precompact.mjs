...
// 導出元の前提:
//
//   閾値での発火    人間が予め定めた地点（context 使用率など）を元に、**半強制的に**会話
//                   引き継ぎを行える
//   引き継ぎの主体  エージェントのネイティブな圧縮・リセット**ではなく**、セッションを跨ぐ
...
// 「これは operator が呼ぶものであって、閾値で自動発火させるものではない」と言う。
...
//   - operator が確認して land するか、あるいは operator が「不要」と言い、次の圧縮は
//     そのまま進む
//
 ...
 * project は体制を採ったことが無く、その儀式を課すことは「operator が決めていないことを
 * plugin が決める」ことになる。**
 */
...
今これを行うこと:
  ...
  2. 何を残し何を省いたかを 1〜2 行で operator に見せ、**land する前に**訂正させること。
  3. land はこれで:  node "$CLAUDE_PLUGIN_ROOT"/bin/handoff.mjs write < <著した.md>

operator が続行を選ぶなら、そう述べて続けること —— これはこのセッションで二度と発火せず、
次の自動圧縮はそのまま進む。`
