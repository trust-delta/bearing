...
> **採るかどうかを決める前に** —— [`docs/adoption.md`](docs/adoption.md) が契約を 1 枚で述べる: **採ると何がリポジトリに置かれ、その後どの act があなたに残り続けるか。**
...
aim に依存しない）。🔴 **corpus が在っても、marker が無ければ黙る。** ⚠ **2026-09-05 まではそうではなく、述語は
`marker || corpus` だった** —— 移行の便宜として、既に node を書いている project を「印が無い」
という理由で黙らせないためである。**同日、述語から corpus が落ちた**（`lib/claude-md.mjs` の
`isEngaged` は `adopted` だけを見る）∴ **corpus が在ることは*使っている証拠*であって、この機構を
通したいという*宣言*ではない。** ⚠ **例外は statusline の 2 行目だけで、corpus を見つけたことを
`aim 未採用` と `corpus N` を 1 行述べる** —— **黙る機構は自分の存在を告げられないからである。**
