...
import { ciSegment, probeCi } from '../../../carriers/claude/bearing/lib/ci.mjs'
import { widthUnsafeChars } from '../../../carriers/claude/bearing/bin/statusline.mjs'
...
test('面は gh を起こさない —— statusline は lib/ci.mjs の read 側しか import しない', async () => {
  const src = await readFile(path.join(import.meta.dirname, '..', '..', '..', 'carriers', 'claude', 'bearing', 'bin', 'statusline.mjs'), 'utf8')
